...
actual_patient_data = pd.read_csv('diabetes_pred.csv')
...
converted_data = pd.get_dummies(actual_patient_data, prefix=['Gender', 'Polyuria', 'Polydipsia', 'sudden weight loss',
                                                             ...
                                                             'partial paresis',
                                                             'muscle stiffness', 'Alopecia', 'Obesity', 'diabetes_class'],
                                drop_first=True)

from sklearn.model_selection import train_test_split

X_train, X_test, y_train, y_test = train_test_split(converted_data.drop('diabetes_class_Positive', axis=1),
                                                    converted_data['diabetes_class_Positive'], test_size=0.3, random_state=0)
...
def predict_note_authentication(age, gender, polyuria, polydipsia, weight, weakness, polyphagia, genital_thrush,
                                visual_blurring, itching, irritability, delayed_healing, partial_paresis,
                                muscle_stiffness, alopecia, obesity,diabetes_class):
    prediction = RF_classifier.predict(sc.transform(np.array([[int(age), int(gender), int(polyuria), int(polydipsia),
                                                               int(weight), int(weakness), int(polyphagia),
                                                               int(genital_thrush), int(visual_blurring), int(itching),
                                                               int(irritability), int(delayed_healing),
                                                               int(partial_paresis), int(muscle_stiffness),
                                                               int(alopecia), int(obesity),int(diabetes_class)]])))
    print(prediction)
    return prediction
...
def main():
    ...
    if obesity == 'Yes':
        ...
        obesity = 0
        diabetes_class = st.radio("Do you have Obesity?", ("es", "No"))
        if diabetes_class == 'Positive':
            diabetes_class = 1
        else:
            obesity = 0
            if polyuria & polydipsia & weight & weakness ==1 & diabetes_class== 'Positive':
                print("diabates type 1 ")

    result = ""
    if st.button("Predict"):
        result = predict_note_authentication(age, gender, polyuria, polydipsia, weight, weakness, polyphagia,
                                             genital_thrush, visual_blurring, itching, irritability, delayed_healing,
                                             partial_paresis, muscle_stiffness, alopecia, obesity,diabetes_class)
        if result == 1:
